<template>
  <div>
    ...
    <v-container v-else>
      ...
      <v-snackbar v-model="passwordSnackbar" top>
        {{ passwordResponse.message }}
        <v-btn color="pink" flat @click="passwordSnackbar = false">Close</v-btn>
      </v-snackbar>
      <h1 class="display-3">
        Hello,
        <span class="blue--text">{{userProfile.name.split(' ')[0]}}.</span>
      </h1>
          ...
          <v-layout row wrap>
            ...
            <v-flex xs12 sm6 class="pb-3">
              <h4 class="title font-weight-regular">
                ...
                >{{userProfile.specialisation}}</span>
              </h4>
            </v-flex>
            <v-flex xs12 sm6 class="pb-3">
              <h4 class="title font-weight-regular">
                Age:
                <span class="blue--text text--darken-3">{{userProfile.age}}</span>
              </h4>
            </v-flex>
            <v-flex xs12 sm6 class="pb-3">
              <h4 class="title font-weight-regular">
                College:
                <span class="blue--text text--darken-3">{{ userProfile.college }}</span>
              </h4>
            </v-flex>
            <v-flex xs12 sm6 class="pb-3">
              <h4 class="title font-weight-regular">
                Email:
                <span class="blue--text text--darken-3">{{userProfile.email}}</span>
              </h4>
            </v-flex>
            <v-flex xs12 sm6 class="pb-3">
              <h4 class="title font-weight-regular">
                Mobile No:
                <span class="blue--text text--darken-3">+91-{{userProfile.pno}}</span>
              </h4>
            </v-flex>
            <v-flex xs12 sm6 class="pb-3">
              <h4 class="title font-weight-regular">
                CGPA:
                <span class="blue--text text--darken-3">{{userProfile.cgpa}}</span>
              </h4>
            </v-flex>
            <v-flex xs12 sm6 class="pb-3">
              <h4 class="title font-weight-regular">
                ...
                <span
                  class="blue--text text--darken-3"
                >{{userProfile.twelfthPercent}}%</span>
              </h4>
            </v-flex>
            <v-flex xs6 sm3>
            ...
            <v-flex xs6 sm3>
              <v-btn flat @click="showDets('approvedJobs')">Jobs Approved
                <v-icon>{{this.det.params == 'approvedJobs' ? 'expand_less' : 'expand_more'}}</v-icon>
              </v-btn>
            </v-flex>
            <v-flex xs6 sm3>
              <v-btn flat @click="showDets('appliedJobs')">Jobs Applied
                <v-icon>{{this.det.params == 'appliedJobs' ? 'expand_less' : 'expand_more'}}</v-icon>
              </v-btn>
            </v-flex>
            <v-flex xs6 sm3>
              <v-btn flat @click="showDets('postedJobs')">Jobs Posted
                <v-icon>{{this.det.params == 'postedJobs' ? 'expand_less' : 'expand_more'}}</v-icon>
              </v-btn>
            </v-flex>
          </v-layout>
          <v-flex xs12>
      ...
      <v-layout column class="pt-5" v-show="det.show">
        ...
        <v-flex xs12>
          <v-alert
            ...
            class="mb-4"
            outline
          >Your job has been successfully deleted. Please reload to see changes.</v-alert>
        </v-flex>
          ...
          <v-card-text>
            <div class="d-flex align-content-space-between">
              <p class="text-xs-left">
                ...
                <span class="grey--text text--darken-1" v-else>{{jobChild.estimatedDuration}} Months</span>
              </p>
              <p class="text-xs-center">Last Date
                <br>
                <span class="grey--text text--darken-1">{{jobChild.lastDate}}</span>
              </p>
              <p class="text-xs-right">No Of Positions Available
                <br>
          ...
          <v-card-actions>
            <v-layout d-flex align-content-space-between>
              <div style="width: 40px;">
                <v-btn flat color="black" @click="jobChild.show=!jobChild.show">More Details
              ...
              <div style="width: 40px;" class="text-xs-right">
                <v-btn
                  flat
                  color="red"
                  v-if="det.params == 'postedJobs'"
                  @click="openDeleteDialog(jobChild)"
                >Delete Job</v-btn>
              </div>
              <div style="width: 40px;" class="text-xs-right">
                <v-btn
                  ...
                  color="blue"
                  v-if="det.params == 'postedJobs'"
                  :to="`/managejob/${jobChild.jobId}`"
                >Manage Applications</v-btn>
              </div>
            </v-layout>
          </v-card-actions>
          <v-slide-y-transition>
            <v-card-text v-show="jobChild.show">{{jobChild.description}}</v-card-text>
          </v-slide-y-transition>
        </v-card>
      </v-layout>
      ...
      <v-dialog
        width="500"
        persistent
        v-model="deleteJobDialog.switch"
        v-if="deleteJobDialog.job != null"
      >
        <v-card>
          ...
          <v-card-text>
            ...
            >By clicking 'Yes' you would be permanently deleting this job. There's no turning back from this.</p>
            <br>
          </v-card-text>
          <v-divider></v-divider>
          <v-card-actions>
